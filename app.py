...
# =============================================================================
# These custom transformers are needed to load and use the preprocessor pipeline
...
if model is not None:
    with st.form("prediction_form"):
        ...
        with col3:
            ...
            screen_size = st.selectbox("Screen Size", ['15" - 15.9"', '14" - 14.9"', '13" - 13.9"', '16" - 16.9"', '17" or more'])
        ...
        # --- Multi-select options in an expander ---
        with st.expander("Additional Features (Connectivity, Multimedia, etc.)"):
            communications = st.multiselect("Communications", ['bluetooth', 'wifi', 'lan 10/100/1000 mbps', 'nfc'], default=['bluetooth', 'wifi'])
            multimedia = st.multiselect("Multimedia", ['camera', 'speakers', 'microphone'], default=['camera', 'speakers', 'microphone'])
            input_devices = st.multiselect("Input Devices", ['keyboard', 'touchpad', 'numeric keyboard', 'illuminated keyboard'], default=['keyboard', 'touchpad'])
            operating_system = st.multiselect("Operating System", ['windows 11 home', 'windows 10 home', 'no system', 'macos'], default=['windows 11 home'])
